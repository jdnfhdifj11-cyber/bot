...
# ====== GITHUB: загрузить файл (через Git Data API для больших файлов) ======
async def github_upload(path: str, content_bytes: bytes, message: str = "upload"):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        # 1. Создаём blob
        blob_url = f"{GITHUB_API}/repos/{GITHUB_USER}/{GITHUB_REPO}/git/blobs"
        blob_payload = {
        ...
        # 2. Получаем текущий HEAD коммит
        ...
        commit_url = f"{GITHUB_API}/repos/{GITHUB_USER}/{GITHUB_REPO}/git/commits/{latest_commit_sha}"
        ...
        tree_url = f"{GITHUB_API}/repos/{GITHUB_USER}/{GITHUB_REPO}/git/trees"
        tree_payload = {
        ...
        # 5. Создаём коммит
        new_commit_url = f"{GITHUB_API}/repos/{GITHUB_USER}/{GITHUB_REPO}/git/commits"
        new_commit_payload = {
            "message": message,
            "tree": new_tree_sha,
            "parents": [latest_commit_sha]
        }
        async with session.post(new_commit_url, headers=HEADERS, json=new_commit_payload) as resp:
            if resp.status != 201:
                return False
        ...
        # 6. Обновляем ref
        update_ref_url = f"{GITHUB_API}/repos/{GITHUB_USER}/{GITHUB_REPO}/git/refs/heads/main"
        async with session.patch(update_ref_url, headers=HEADERS, json={"sha": new_commit_sha}) as resp:
...
async def got_apk(message: Message, state: FSMContext):
    ...
    if ok2:
        ...
        import io
        html_file = io.BytesIO(html.encode())
        html_file.name = f"{landing_id}.html"
